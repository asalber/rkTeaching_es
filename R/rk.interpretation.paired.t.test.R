#' rk.interpretation.paired.t.test
#' Function that prints the interpretation of the t.test for two paired populations procedure in HTML format.
#'
#' @param result is the result of the paired t.test procedure.
...
#'
#' @examples
rk.interpretation.paired.t.test <- function (result, conf.int=TRUE){
  require(R2HTML)
  conclusion <- NULL
  ...
  if (result$p.value < 0.01){ 
    ...
    interpretation.test <- sprintf(
      'Como el p-valor del test T para la media poblacional de la diferencia %f es menor que 0.01 <em>se rechaza la hipótesis nula con un nivel de significación 0.01</em> y se concluye que <em>hay pruebas muy significativas de que la media de la variable %s es %s la media de la variable %s</em>.<br/>
Observe, no obstante, que una diferencia estadísticamente significativa no necesariamente supone una diferencia relevante en el dominio de aplicación.',
      result$p.value,
    ...
    interpretation.test <- sprintf(
      'Como el p-valor del test T para la media poblacional de la diferencia %f es menor que 0.05 <em>se rechaza la hipótesis nula 
con un nivel de significación 0.05</em> y se concluye que <em>hay pruebas significativas de que la media 
de la variable %s es %s la media de la variable %s</em>.<br/>
...
una diferencia relevante en el dominio de aplicación.', 
    ...
    interpretation.test <- sprintf(
      'Como el p-valor del test T para la media poblacional de la diferencia %f es mayor que 0.05 <em>no se puede rechazar 
la hipótesis nula con un nivel de significación 0.05</em> y se concluye que <em>no hay pruebas significativas de que la media 
de la variable %s es %s la media de la variable %s</em>.',
      result$p.value,
      strsplit(result$data.name, "\\\"")[[1]][2],
      ifelse(result$alternative == "two.sided", "distinta de", ifelse(result$alternative == "greater", "mayor que", "menor que")),
      strsplit(result$data.name, "\\\"")[[1]][4]
  ...
  if (conf.int){
    if (result$alternative == "two.sided"){
      interpretation.int <- sprintf(
        'El intervalo de confianza indica además que la media poblacional de la diferencia entre la variable %s y la variable %s 
está entre %f y %f con una confianza del %i%%.',
        strsplit(result$data.name, "\\\"")[[1]][2],
        strsplit(result$data.name, "\\\"")[[1]][4],
        result$conf.int[1],
        result$conf.int[2],
      ...
      interpretation.int <- sprintf(
        'El intervalo de confianza indica además que la media poblacional de diferencia entre la variable %s y la variable %s es
 mayor que %f con una confianza del %i%%.',
        strsplit(result$data.name, "\\\"")[[1]][2],
        strsplit(result$data.name, "\\\"")[[1]][4],
        result$conf.int[1],
        attr(result[["conf.int"]],"conf.level")*100
      ...
      interpretation.int <- sprintf(
        'El intervalo de confianza indica además que la media poblacional de diferencia entre la variable %s y la variable %s es 
menor que %f con una confianza del %i%%.',
        strsplit(result$data.name, "\\\"")[[1]][2],
        strsplit(result$data.name, "\\\"")[[1]][4],
        result$conf.int[2],
        attr(result[["conf.int"]],"conf.level")*100
